      ...
      <div className="grid grid-cols-12 gap-4 p-0 md:p-8 md:w-4/5 w-full h-[90%]">
        <div className="lg:col-span-8 col-span-12 py-8 px-4 md:p-8 bg-main">
          <Image
            ...
            loading="lazy"
          />
          <h1 className="text-2xl text-title font-bold my-4 md:my-8">
            {props.title}
          </h1>
          <div className="flex align-center justify-between text-sub pb-4 md:pb-8">
            <time dateTime={props.updatedAt}>
              {dayjs(props.updatedAt).format("YYYY-MM-DD")}
            </time>
            {props.category && <p className="text-sub">{props?.category.id}</p>}
          </div>
